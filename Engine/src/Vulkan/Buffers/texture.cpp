...
namespace SnowEngine {
    ...
    Texture::~Texture() {        
        ...
        vmaDestroyImage(device, textureImage, allocation);
    }
    ...
    void Texture::CreateTextureImage(const std::string& path) {
        ...
        device.CreateBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, &stagingAllocation);

        void* tmp; //metto i pixel dell`immagine in un buffer
        vmaMapMemory(device, stagingAllocation, &tmp);
        memcpy(tmp, pixels, static_cast<size_t>(imageSize));
        vmaUnmapMemory(device, stagingAllocation);

        stbi_image_free(pixels);

        device.CreateImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &textureImage, &allocation);

        TransitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        CopyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        TransitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

        vmaDestroyBuffer(device, stagingBuffer, stagingAllocation);
    }
